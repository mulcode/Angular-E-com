...
export class ProductService {
  ...
  addCarts(cartData: cart) {
    return this.http.post('http://localhost:3000/carts', cartData);
  }
  getCartList(userId: number) {
    return this.http
      .get<product[]>('http://localhost:3000/carts?userId=' + userId, {
        observe: 'response',
      })
      .subscribe((result) => {
        console.warn(result);
  ...
  currentCart() {
    ...
    return this.http.get<cart[]>(
      'http://localhost:3000/carts?userId=' + userData.id
    );
  }
  ...
  deleteCartItems(cartId: number) {
    return this.http
      .delete('http://localhost:3000/carts/' + cartId, { observe: 'response' })
      .subscribe((result) => {
        if (result) {
          this.cartData.emit([]);
        }
